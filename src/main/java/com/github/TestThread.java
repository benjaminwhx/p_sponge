...
import com.github.sponge.SpongeThreadPoolExecutor;
...
public class TestThread {
	...
	public static void main(String agrs[]) {
		try {
			...
			String tmpDir = "/Users/Benjamin/Desktop/threadPoolTest";
			if (agrs.length > 0) {
				tmpDir = agrs[0];
			}
			
			HashMap map = new HashMap();
			map.put(SpongeThreadPoolExecutor.FILE_PERSISTENCE_DIR, tmpDir);
			ThreadPoolExecutor tmpThreadPool = SpongeThreadPoolExecutor.generateThreadPoolExecutor(
					10, 50, 60L, TimeUnit.SECONDS, map);
			...
			for (int j = 0; j < 1000; j++) {
				...
				System.out.println("第 "+j+" 次放 "+tmpOneTimeCnt+" 个请求到线程池耗时 " + (System.currentTimeMillis() - tmpStartTime));
