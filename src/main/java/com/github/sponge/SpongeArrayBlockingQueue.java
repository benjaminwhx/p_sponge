...
public class SpongeArrayBlockingQueue<E> extends AbstractQueue<E>
     ...
     * @param oneBatchSize the queue once batch persist size
	 * @throws IllegalArgumentException if <tt>capacity</tt> is less than 1
	 */
	public SpongeArrayBlockingQueue(int capacity, int oneBatchSize,
			SpongeService spongeService) throws Exception {
		if (oneBatchSize >= capacity) {
			throw new SpongeException("the queue once batch persist size can't > queue capacity");
		}
		...
		theMemoryItemList = new MemoryItemList(oneBatchSize, spongeService);
	}
	...
	public SpongeArrayBlockingQueue(int capacity, boolean fair,
		...
		theMemoryItemList = new MemoryItemList(oneBatchSize,
				spongeService);
	}
	...
	public SpongeArrayBlockingQueue(int capacity, boolean fair,
							  ...
							  int oneBatchSize,
							  SpongeService spongeService) {
		this(capacity, fair, oneBatchSize, spongeService);
		if (capacity < c.size()) {
			throw new IllegalArgumentException();
		}
	...
	public boolean offer(E e) {
		...
		try {
			// 如满足下面条件,则进行持久化保存
			if (count == items.length || isInPersistence) {
	 ...
	 */
	public void initFetchData(ThreadPoolExecutor threadPoolExecutor) {
		final ReentrantLock lock = this.lock;
		lock.lock();
		try {
			theMemoryItemList.initFetchData(threadPoolExecutor);
		} finally {
			lock.unlock();
		}
	}
	...
	class MemoryItemList {
		...
		protected MemoryItemList(int oneBatchSize, SpongeService spongeService) {
			this.oneBatchSize = oneBatchSize;
			itemArray = new Object[oneBatchSize];
			this.persistence = spongeService.getThePersistence();
		}
         ...
         * @return
         */
		protected boolean addOneRequest(Object request) {
			itemArray[count] = request;
			count++;
		...
		private void generateOneBatchBytes() {
			...
			try {
				...
				persistence.persistData(theBytesOut.getDataClone());
			} catch (Exception ex) {
				//
			} finally {
				count = 0;
		...
		private void initFetchData(ThreadPoolExecutor threadPoolExecutor) {
			if (persistence.haveDataInPersistence()) {
				try {
					byte[] tmpBytes = persistence.fetchDataFromPersistence();
					...
					if (tmpBytes != null) {
						...
						while (tmpCurPosi < tmpBytes.length) {
							...
							threadPoolExecutor.execute((Runnable)tmpOneItem);
